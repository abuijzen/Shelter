...
namespace Shelter.Mvc
{
    ...
    public class ShelterDataAccess : IShelterDataAccess
    {
        ...
        {
			//zoek shelter op id
            var shelter = _context.Shelters.Include(x => x.Animals).FirstOrDefault(x => x.Id == shelterId);
            ...
            else
            {
                ...
                if (animals.Count > 1)
                {
					...
					var animal = _context.Animals.FirstOrDefault(x => x.Id == animals.FirstOrDefault().Id);
					return animal;
				}
				return default(Animal);
            }
